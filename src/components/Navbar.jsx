...
/**
 * Header fix (sticky) care conține:
 *  • o bară utilitară (telefon / program / e-mail) care se pliază la derulare
 *  • navigația pe desktop, cu subliniere animată și evidențiere scroll-spy
 *  • un sertar mobil glisant (buton burger, Esc pentru închidere, clic pe fundal,
 *    blocarea derulării paginii și închidere automată la navigare)
 */
export default function Navbar() {
  const [open, setOpen] = useState(false)
  const scrolled = useScrolled(20)

  // Memoizat pentru ca observatorul din interiorul hook-ului să nu fie reconstruit la fiecare randare.
  const sectionIds = useMemo(() => navLinks.map((l) => l.href.slice(1)), [])
  const active = useActiveSection(sectionIds)

  useBodyScrollLock(open)

  // Tasta Escape închide sertarul — comportamentul așteptat pentru orice overlay.
  useEffect(() => {
    if (!open) return
  ...
  // Se închide automat dacă viewportul depășește breakpointul pentru mobil.
  useEffect(() => {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      {/* ---------- Bară utilitară: se comprimă la înălțime 0 după derulare ---------- */}
      <div
            ...
            {navLinks.map((link) => {
              ...
              return (
                <li key={link.href}>
                  ...
                  >
                    {link.label}
                    {/* Sublinierea crește din centru la hover / când linkul este activ */}
                    <span
      ...
      <aside
        ...
        aria-hidden={!open}
        /* `inert` scoate sertarul închis din ordinea de tabulare — acesta este doar
           translatat în afara ecranului, nu ascuns cu display:none. */
        inert={!open}
      ...
      >
        ...
        <nav aria-label="Meniu mobil" className="flex-1 overflow-y-auto px-3 py-4">
