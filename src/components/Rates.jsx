...
/**
 * Blocul de prețuri, în două forme coordonate:
 *  • trei carduri de produs (la toate breakpoint-urile) — cel din mijloc este evidențiat
 *  • un tabel comparativ complet (doar lg+), care ar fi ilizibil pe mobil,
 *    unde cardurile transmit deja aceleași informații
 */
  ...
  return (
    <section id="rates" className="section bg-ink-50/60">
      <div className="container-page">
        <SectionHeading
          eyebrow="Dobânzi indicative"
          eyebrowIcon="percent"
          title="Cele mai bune dobânzi de astăzi din"
          accent="portofoliul nostru de finanțatori"
          lead="Actualizate săptămânal. Acestea sunt dobânzile pe care clienții noștri le primesc efectiv luna aceasta — nu cifre de vitrină, rezervate dosarelor perfecte."
        />
          ...
          {rateCards.map((card, i) => (
            ...
            >
              ...
              <p className={`mt-2 text-xs ${card.featured ? 'text-brand-200/80' : 'text-ink-500'}`}>
                DAE reprezentativă {card.apr}% · dobândă pe perioada fixă
              </p>
              ...
              >
                <div className="flex justify-between gap-3">
                  <dt className={card.featured ? 'text-brand-200' : 'text-ink-500'}>Sumă</dt>
                  <dd className={`text-right font-semibold ${card.featured ? 'text-white' : 'text-ink-800'}`}>
                    {card.amount}
                  </dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className={card.featured ? 'text-brand-200' : 'text-ink-500'}>Perioadă</dt>
                  <dd className={`text-right font-semibold ${card.featured ? 'text-white' : 'text-ink-800'}`}>
                    {card.term}
                  </dd>
                </div>
              ...
              >
                Verifică-mi eligibilitatea
              </Button>
            </Reveal>
          ))}
        </div>

        {/* ---------- Tabel comparativ (doar desktop) ---------- */}
        <Reveal delay={120} className="mt-14 hidden lg:block">
          <div className="overflow-hidden rounded-3xl border border-ink-200/70 bg-white shadow-soft">
            <table className="w-full text-left text-sm">
              <caption className="sr-only">Comparație alăturată a produselor de creditare FlorinFinance</caption>
              <thead>
